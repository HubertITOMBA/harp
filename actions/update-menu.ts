...
export async function updateMenu(menuId: number, formData: FormData) {
  try {
    ...
    // Si le nom du menu change, vérifier qu'il n'existe pas déjà
    if (existingMenu.menu !== validatedData.menu) {
      const duplicateMenu = await db.harpmenus.findUnique({
        where: {
    ...
    return { 
      success: true, 
      message: `Le menu ${validatedData.menu} a été mis à jour avec succès` 
    };
  } catch (error) {
