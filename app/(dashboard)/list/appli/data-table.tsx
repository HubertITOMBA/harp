    ...
    <>
     ...
     <div className="flex items-center py-3 w-full max-w-2xl">
        <Input
          ...
          value={(table.getColumn("appli")?.getFilterValue() as string) ?? ""}
          onChange={(event) =>
            table.getColumn("appli")?.setFilterValue(event.target.value)
          }
          ...
          {table.getRowModel().rows?.length ? (
            table.getRowModel().rows.map((row) => (
              <TableRow
                ...
                className="hover:bg-harpSkyLight transition-colors durée-150 h-7 sm:h-8"
              >
