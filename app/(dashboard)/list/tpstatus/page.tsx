...
import { CreateTypeStatusDialog } from '@/components/typstatus/CreateTypeStatusDialog';
        ...
        <Card className="shadow-xl border-0 bg-white/80 backdrop-blur-sm">
          <CardHeader className="harp-card-header">
            <CardTitle className="flex items-center gap-3 text-xl sm:text-2xl lg:text-3xl font-bold">
              <Flag className="h-6 w-6 sm:h-7 sm:w-7 lg:h-8 lg:w-8" />
              Les statuts
            </CardTitle>
            <p className="text-orange-50 text-sm sm:text-base mt-2">
              {statusCount} statut{statusCount > 1 ? "s" : ""} enregistré{statusCount > 1 ? "s" : ""}
            </p>
          </CardHeader>
          <CardContent className="p-4 sm:p-6">
            <div className="mb-4 flex justify-end">
              <CreateTypeStatusDialog />
            </div>
