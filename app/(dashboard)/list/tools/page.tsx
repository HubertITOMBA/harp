...
import { CreateToolsDialog } from '@/components/tools/CreateToolsDialog';

const ToolsListPage = async () => {
  const data = await db.harptools.findMany({
    orderBy: {
      descr: 'asc',
    },
    select: {
        ...
        <Card className="shadow-xl border-0 bg-white/80 backdrop-blur-sm">
          <CardHeader className="harp-card-header">
            ...
            <p className="text-orange-50 text-sm sm:text-base mt-2">
              {toolsCount} outil{toolsCount > 1 ? "s" : ""} enregistré{toolsCount > 1 ? "s" : ""}
            </p>
          </CardHeader>
