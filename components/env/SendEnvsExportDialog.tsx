...
import { sendEnvsExportByEmail } from '@/lib/actions/export-envs-actions';
import { getAllUsersForNotifications, getAllRolesForNotifications } from '@/lib/actions/notification-actions';
import { toast } from 'react-toastify';
import { MultiSelect, MultiSelectOption } from '@/components/ui/multi-select';
import { EnvData } from '@/lib/export-envs';

interface SendEnvsExportDialogProps {
  envs: EnvData[];
  visibleColumns: string[];
}

export function SendEnvsExportDialog({ envs, visibleColumns }: SendEnvsExportDialogProps) {
  ...
  const handleSubmit = async (
    ...
    startTransition(async () => {
      const result = await sendEnvsExportByEmail(formData);
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Input
            id="subject"
            name="subject"
            defaultValue={`Export des environnements - ${new Date().toLocaleDateString('fr-FR')}`}
            className="bg-white"
        ...
        <div className="space-y-2">
          ...
          <Textarea
            ...
            name="message"
            rows={4}
            defaultValue={`Veuillez trouver ci-joint l'export des environnements (${envs.length} environnement${envs.length > 1 ? 's' : ''}).`}
            className="bg-white"
