...
export function CreateMenuDialog() {
  ...
  const handleSubmit = async (
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        ...
        toast.error(result.error || "Erreur lors de la création du menu");
        if (result.error) {
  ...
  return (
    <FormDialog
      ...
      description="Remplissez les informations pour créer un nouveau menu"
      onSubmit={handleSubmit}
      submitLabel="Créer le menu"
      submitIcon={<Menu className="h-4 w-4" />}
      isPending={isPending}
      maxWidth="2xl"
    >
