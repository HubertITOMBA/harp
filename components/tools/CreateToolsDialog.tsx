...
import { Wrench, Plus, FolderOpen } from "lucide-react";
import { createTools } from '@/actions/create-tools';
...
export function CreateToolsDialog() {
  ...
  const handleSubmit = async (
    ...
    const toolValue = formData.get("tool") as string;
    const validationErrors: Record<string, string> = {};
    
    if (!toolValue || toolValue.trim() === "") {
    ...
    startTransition(async () => {
      const result = await createTools(formData);
      
      if (result.success) {
        toast.success(result.message || "L'outil a été créé avec succès", {
          position: "top-right",
          autoClose: 3000,
        });
        closeDialog();
        ...
        const errorMessage = result.error || "Une erreur est survenue lors de la création de l'outil";
  ...
  return (
    <FormDialog
      ...
      description="Remplissez les informations pour créer un nouvel outil"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            placeholder="Ex: PSADMIN (obligatoire)"
            maxLength={255}
            onChange={(e) => {
              if (errors.tool) {
                setErrors(prev => {
