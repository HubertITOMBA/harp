...
import { sendTaskByEmail } from '@/lib/actions/send-task-email-actions';
import { getAllUsersForNotifications, getAllRolesForNotifications } from '@/lib/actions/notification-actions';
import { toast } from 'react-toastify';
import { MultiSelect, MultiSelectOption } from '@/components/ui/multi-select';
import { Task } from '@/lib/export-task';

interface SendTaskEmailDialogProps {
  task: Task;
}

export function SendTaskEmailDialog({ task }: SendTaskEmailDialogProps) {
  ...
  const handleSubmit = async (
    ...
    startTransition(async () => {
      const result = await sendTaskByEmail(formData);
  ...
  return (
    <FormDialog
      ...
      title="Envoyer la chrono-tâche par email"
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Input
            id="subject"
            name="subject"
            defaultValue={`Chrono-tâche: ${task.title} - ${new Date().toLocaleDateString('fr-FR')}`}
            className="bg-white"
        ...
        <div className="space-y-2">
          ...
          <Textarea
            ...
            name="message"
            rows={4}
            defaultValue={`Veuillez trouver ci-joint la chrono-tâche "${task.title}" (${task.items?.length || 0} tâche${(task.items?.length || 0) > 1 ? 's' : ''}).`}
            className="bg-white"
