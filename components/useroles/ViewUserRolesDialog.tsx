...
import { ViewUserRolesContent } from './ViewUserRolesContent';
import { getUserRolesById } from '@/lib/actions/useroles-actions';

interface ViewUserRolesDialogProps {
  netid: string;
  role: string;
  userName: string;
  roleName: string;
...
export function ViewUserRolesDialog({ netid, role, userName, roleName }: ViewUserRolesDialogProps) {
  ...
  useEffect(() => {
    if (open && !userRoleData) {
      setLoading(true);
      getUserRolesById(netid, role)
        .then((data) => {
          setUserRoleData(data);
        })
        .catch((error) => {
          console.error("Erreur lors du chargement de l'attribution de rôle:", error);
        })
        .finally(() => {
          setLoading(false);
        });
    }
  }, [open, netid, role, userRoleData]);
      ...
      <DialogContent className="max-w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="py-4">
          {loading ? (
            ...
            <ViewUserRolesContent userRole={userRoleData} />
          ) : (
